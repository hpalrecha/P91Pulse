...
export default function DistributorClaimFormPage() {
  ...
  // Load current user, detailers, and warranties
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Get warranties
        const warrantiesResponse = await apiRequest('GET', '/api/erp/distributor/warranties');
        ...
        setDetailers(detailersData.filter((d: any) => d.status === 'approved'));
  ...
  // Filter warranties when detailer selection changes
  useEffect(() => {
    if (selectedDetailerId) {
      const detailerId = parseInt(selectedDetailerId);
      const filtered = warranties.filter(warranty => 
        warranty.detailerId === detailerId
      );
      setFilteredWarranties(filtered);
      setSelectedWarranty(null);
      
      // Set the detailer ID in form
      form.setValue('detailerId', detailerId);
    } else {
      setFilteredWarranties(warranties);
      setSelectedWarranty(null);
    }
  }, [selectedDetailerId, warranties, form]);
  ...
  const handleWarrantySelect = (warranty: any) => {
    ...
    form.setValue('detailerId', warranty.detailerId);
    
    // Update detailer select if not already set
    if (!selectedDetailerId) {
      setSelectedDetailerId(warranty.detailerId.toString());
    }
  };
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'The warranty claim has been submitted successfully.',
      });
    ...
    <div className="py-6 px-4 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Create a new warranty claim for a detailer's product.
        </p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Detailer & Warranty Selection */}
        <Card className="md:col-span-1">
          <CardHeader>
            <CardTitle>Select Detailer & Warranty</CardTitle>
            <CardDescription>
              Choose the detailer and warranty record for the claim
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Detailer Selection */}
            <div>
              <Label>Select Detailer</Label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="">All Detailers</SelectItem>
            ...
            {filteredWarranties.length === 0 ? (
              <Alert>
                ...
                <AlertDescription>
                  {selectedDetailerId 
                    ? "This detailer doesn't have any active warranties in the system." 
                    : "No active warranties found. Please select a detailer."}
                </AlertDescription>
              </Alert>
            ) : (
                    ...
                    >
                      <CardContent className="p-4">
                        ...
                        <div className="text-xs text-gray-400">
                          Detailer: {detailers.find(d => d.id === warranty.detailerId)?.name || 'Unknown'}
                        </div>
                      </CardContent>
                    </Card>
                  ))}
